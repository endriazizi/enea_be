...
_Generato: Tue, Oct 28, 2025  1:40:11 AM_
...
/**
 * Router ORDERS
 * - GET /api/orders            → lista con filtri server-side (status, hours, q)
 * - POST /api/orders           → crea ordine (testa + righe)
 * - GET  /api/orders/:id       → dettaglio (con items)
 * - PATCH /api/orders/:id/status → cambio stato
 * - GET  /api/orders/stream    → SSE live (created/status)
 *
 * NOTE: mantiene il tuo stile (log con emoji) e la compatibilità con il FE.
 */
...
const logger  = require('../logger');
const Orders  = require('../services/orders.service');     // service principale
const OrdersSSE = require('../services/orders.sse');       // broadcaster SSE

// ================================ LIST =======================================
// GET /api/orders?status=&hours=&q=
router.get('/', async (req, res) => {
  try {
    const raw = req.query || {};

    const filter = {
      status: (raw.status || '').toString().trim() || undefined,  // 'pending' | 'confirmed' | ...
      hours : Number(raw.hours || 0) || undefined,                // es: 6 → ultime 6 ore
      q     : (raw.q || '').toString().trim() || undefined        // search: id/nome/phone/email/channel
    };

    logger.info('📥 [GET] /api/orders (filtri server-side)', { filter });
...
router.post('/', async (req, res) => {
  try {
    ...
    logger.info('➕ [POST] /api/orders OK', { id: created.id, total: created.total });

    // SSE: broadcast "created"
    OrdersSSE.emitCreated(created);
...
router.get('/:id(\\d+)', async (req, res) => {
  ...
  try {
    const full = await Orders.getById(id);
...
router.patch('/:id(\\d+)/status', async (req, res) => {
  ...
  try {
    ...
    logger.info('✏️ [PATCH] /api/orders/:id/status OK', { id, status: updated.status });

    // SSE: broadcast "status"
    OrdersSSE.emitStatus({ id: updated.id, status: updated.status });

    res.json(updated);
  } catch (err) {
    logger.error('💥 [PATCH] /api/orders/:id/status', { id, error: String(err) });
    res.status(400).json({ error: err.message || 'invalid_status' });
  }
});

// ================================ STREAM =====================================
router.get('/stream', (req, res) => {
  OrdersSSE.handleStream(req, res);
});
...
/**
 * Service ORDERS
 * - list(filter) con filtri server-side (status, hours, q)
 * - create(dto)  testa + righe + totale
 * - getById(id)  con items
 * - updateStatus(id, status)
 *
 * NOTE: compat con la tua migrazione finale:
 *   orders:  id, customer_name, phone, email, people, scheduled_at, note,
 *            channel, status, total, created_at, updated_at
 *   order_items: id, order_id, product_id, name, qty, price, notes, created_at
 */
...
function fmtDateTime(d) {
  // YYYY-MM-DD HH:mm:ss (locale server)
...
async function list(filter = {}) {
  ...
  // status
  if (filter.status && ALLOWED_STATUS.has(filter.status)) {
    where.push('o.status = ?');
    params.push(filter.status);
  }

  // hours -> calcolo timestamp da JS (evita INTERVAL ? HOUR)
  if (filter.hours && Number(filter.hours) > 0) {
    const h = Math.min(24 * 30, Number(filter.hours)); // safety: max 30 giorni
    const since = new Date(Date.now() - h * 3600 * 1000);
    where.push('o.created_at >= ?');
    params.push(fmtDateTime(since));
  }

  // q: se numero → match id. Sempre anche LIKE su nome/phone/email/channel
  if (filter.q) {
    const q = String(filter.q).trim();
    const idNum = Number(q);
    const like = `%${q}%`;
    const or = ['o.customer_name LIKE ?', 'o.phone LIKE ?', 'o.email LIKE ?', 'o.channel LIKE ?'];
    params.push(like, like, like, like);
    if (Number.isFinite(idNum)) {
      or.unshift('o.id = ?');
      params.unshift(idNum);
    }
  ...
  const sql = `
    SELECT
      o.id, o.customer_name, o.phone, o.email, o.channel,
  ...
  logger.info('📄 [ORDERS.list] SQL', { where, paramsCount: params.length });
...
async function create(dto) {
  // guardie minime
  const name = (dto.customer_name || '').toString().trim();
  ...
  const people = Number(dto.people || 0) || null;
  const scheduled_at = (dto.scheduled_at || null);
  const note = (dto.note || null);
  const channel = (dto.channel || 'admin');
  ...
  try {
    ...
    const [insHead] = await conn.query(
      `INSERT INTO orders
       (customer_name, phone, email, people, scheduled_at, note, channel, status, total)
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [name, phone, email, people, scheduled_at, note, channel, 'pending', total]
    );
    const orderId = insHead.insertId;
    ...
    if (items.length) {
      ...
      const sqlItems = `
        ...
        VALUES ${values.join(',')}
      `;
      await conn.query(sqlItems, params);
    }

    await conn.commit();

    logger.info('🧾 [ORDERS.create] OK', { id: orderId, items: items.length, total });

    // ritorno testata (coerente con FE)
    return {
      id: orderId,
      customer_name: name,
      phone, email, channel,
    ...
    await conn.rollback();
    logger.error('💥 [ORDERS.create] rollback', { error: String(err) });
    throw err;
  } finally {
    conn.release();
  }
...
async function getById(id) {
  const [hRows] = await pool.query(
    `SELECT id, customer_name, phone, email, people, scheduled_at, note, channel, status, total, created_at, updated_at
  ...
  const [iRows] = await pool.query(
    `SELECT id, order_id, product_id, name, qty, price, notes, created_at
  ...
  return {
    ...
    updated_at: h.updated_at ? new Date(h.updated_at).toISOString() : null,
    items: iRows.map(r => ({
      id: r.id,
      order_id: r.order_id,
...
async function updateStatus(id, nextStatus) {
  ...
  if (!ALLOWED_STATUS.has(status)) throw new Error('status_not_allowed');
  ...
  logger.info('🔁 [ORDERS.updateStatus] OK', { id, status });

  // ritorna testata coerente con FE (senza items)
  const [rows] = await pool.query(
    `SELECT id, customer_name, phone, email, channel, status, total, created_at, updated_at
     FROM orders WHERE id = ?`,
    [id]
  );
